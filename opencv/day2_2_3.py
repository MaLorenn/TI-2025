...
while True:
    ...
    if hierarchy is not None and len(contours) > 0:
        ...
        for i, contour in enumerate(contours):
            if hierarchy[0][i][3] != -1:
                ...
                if len(approx) == 4:
                    ...
                    if 2500 <= area <= 70000 and area > max_area:
                        max_area = area
        ...
        if approx_inner is not None:
            ...
            region = cv2.bitwise_and(gray_img, gray_img, mask=mask)
            region_pixels = region[mask == 255]
            black_ratio = np.sum(region_pixels < 90) / len(region_pixels) if len(region_pixels) > 0 else 0.0
            ...
            if black_ratio > 0.04:
                ...
                pts_real = np.float32([[0, 0], [width, 0], [width, height], [0, height]])
                ...
                if not (np.isnan(cx) or np.isnan(cy)):
                    ...
                    for edge in edges:
                        for direction in [1, -1]:
                            p = line_intersection((cx, cy), (cx + direction * 1000 * v[0], cy + direction * 1000 * v[1]), edge[0], edge[1])
                            if p is not None:
                                hits.append(p)
                    ...
                    if len(hits) >= 2:
                        ...
                        p1, p2 = hits[:2]
                        if np.dot(p1 - [cx, cy], v) > np.dot(p2 - [cx, cy], v):
                            down_pt, up_pt = p1, p2
                        else:
                            down_pt, up_pt = p2, p1
                        vec_v = down_pt - np.array([cx, cy])
                        len_v = np.linalg.norm(vec_v)
                        dir_v = vec_v / (len_v + 1e-6)

                        v_h = pts[1] - pts[0]
                        v_h = v_h / (np.linalg.norm(v_h) + 1e-6)
                        h_hits = []

                        for edge in edges:
                            for direction in [1, -1]:
                                p = line_intersection((cx, cy), (cx + direction * 1000 * v_h[0], cy + direction * 1000 * v_h[1]), edge[0], edge[1])
                                if p is not None:
                                    h_hits.append(p)

                        if len(h_hits) >= 2:
                            h_hits = sorted(h_hits, key=lambda pt: np.linalg.norm(pt - np.array([cx, cy])))
                            p1_h, p2_h = h_hits[:2]
                            if np.dot(p1_h - [cx, cy], v_h) > np.dot(p2_h - [cx, cy], v_h):
                                right_pt, left_pt = p1_h, p2_h
                            else:
                                right_pt, left_pt = p2_h, p1_h
                            vec_h = right_pt - np.array([cx, cy])
                            len_h = np.linalg.norm(vec_h)
                            dir_h = vec_h / (len_h + 1e-6)
                            #更改target坐标
                            offset_v = (90 / 174) * len_v
                            offset_h = (94 / 261) * len_h
                            target_pt = np.array([cx, cy]) + dir_v * offset_v + dir_h * offset_h
    ...
    if elapsed >= 1.0:
        ...
        fps_time = now_fps
        print(f"FPS: {fps:.1f}")
    cv2.putText(img_raw, f"FPS: {fps:.1f}", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
